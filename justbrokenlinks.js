...
async function checkurlshp(baseURL) {
    const resp = await fetch(baseURL);
    const htmlBody = await resp.text();
    var nextURLs = getURLsfromHTMLhp(htmlBody, baseURL);
...
function getURLsfromHTMLhp(htmlBody, baseURL) {
    var newBaseURL = null;
    const urls = [];
    const dom = new JSDOM(htmlBody);
    dom.window.document.querySelectorAll('nav a').forEach((a) => {
        ...
        if ((href.slice(0, 1) === '/') || (href.slice(0, 1) === '#')) {
            ...
            try {
                ...
                urls.push(`${a.text}#${urlobj.href.split('https://')[1]}`);
            } catch (err) {
